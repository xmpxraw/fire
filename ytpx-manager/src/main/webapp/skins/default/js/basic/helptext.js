...
Namespace('basic.helptext',{
	init:function() {
		easyui.util.initDatagrid('#datagrid',{
			pageSize:20,
			onDblClickRow:function() {
				basic.helptext.update();
			}
		});
	},
	query:function() {
		getDialog('query-dialog').dialog({
			href:contextpath+'/basic/helptext/query',
			title: '查询帮助说明',
			width: 350,
		    height: 250,
		    buttons:[{
	...
	update:function() {
		...
		getDialog('data-dialog').dialog({
			href:contextpath+'/basic/helptext/update/'+row.id,
			title: '修改帮助说明',
			width: 650,
		    height: 450,
		    buttons:[{
	...
	doUpdate:function() {
		...
		var row = $('#datagrid').datagrid('getSelected');
		$.postJson(contextpath+'/basic/helptext/update/'+row.id,data,function(result) {
	...
	add:function() {
		getDialog('data-dialog').dialog({
			href:contextpath+'/basic/helptext/add',
			title: '添加帮助说明',
			width: 650,
		    height: 450,
		    buttons:[{
	...
	doAdd:function() {
		...
		$('#datagrid').datagrid('loading');
		$.postJson(contextpath+'/basic/helptext/add',data,function(result) {
			$('#datagrid').datagrid('loaded');
			if(result.code==200) {
				//top.showInfo('添加成功!');
				$('#datagrid').datagrid('reload');
	...
	remove:function() {
		...
		$.messager.confirm('信息提示','确定要删除吗？',function(sure){
		    if(!sure) return;
		    $('#datagrid').datagrid('loading');
		    $.post(contextpath+'/basic/helptext/delete/'+row.id,function(result) {
				$('#datagrid').datagrid('loaded');
				if(result.code==200) {
					$('#datagrid').datagrid('reload');
				} else {
	...
	sort:function() {
		getDialog('sort-dialog').dialog({
			href:contextpath+'/basic/helptext/sort',
			title: '排序',
		    width: 400,
		    height: 400,
		    buttons:[{
	...
	doSort:function() {
		...
		$.postJson(contextpath+'/basic/helptext/sort',rows,function(result) {
			$('#datagrid').datagrid('loaded');
			if(result.code==200) {
				$('#datagrid').datagrid('reload');
			} else {
