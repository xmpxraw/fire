<template>
  <div class="page-xWrongPrt">
    <header-nav title="错题练习" :backshow="backshow"></header-nav>
    <div class="contain" v-show="allCon">
      ...
      <div class="submit" v-show="submitBtn" data-id={{questionAllData.questionId}} @click="submitPro">提交</div>
      <footer>
        <p v-show="false" class="conRight"><span>{{rightCount}}对/{{wrongCount}}错</span><br><span>{{unAnswerCount}}未答</span></p>
    ...
    <div class="mask" v-show="showMenu">
      <div class="problemDetail">
        <div class="mask_top">
          <p>
            <span v-show="false">答对{{rightCount}}</span> <span v-show="false">答错{{wrongCount}}</span> <span v-show="false">未答{{unAnswerCount}}</span> <span @click="showMenu=false">取消</span>
          </p>
          <p>题号</p>
        </div>
...
//        xxx: 跟章节练习接口不一样
        apiService.getAllWrongQuestion(perparams).then((data) => {
          this.loading = false;
...
//            xxx: 这里跟章节练习不一样 id 改为 questionId
            this.momoList[this.questionList[i].questionId]=i;
